// get all Platforms from database in a cache.
import React, { Component } from 'react';
import TextField from 'material-ui/TextField';
import Paper from 'material-ui/Paper';
import FlatButton from 'material-ui/FlatButton';
import Divider from 'material-ui/Divider';
import cache from '../../../data/platformsCache';
...
class PlatformsSandbox extends Component {
  ...
  onFormSubmit(event) {
    event.preventDefault();
      ...
      if (this.state.platform !== '') options.console_name = this.state.platform;
  ...
  onPlatformChange(event) {
    ...
    if (cache[event.target.value]) {
      this.setState({ errorPlatform: '' });
    }
    else {
      this.setState({ errorPlatform: 'Invalid Platform' });
    }
  }
  ...
  render() {
    return (
      ...
      <form className="Form col-md-4 row" >
        ...
        <TextField
          ...
          <TextField
            ...
            hintText="Platform"
            errorText={this.state.errorPlatform}
            underlineShow={false}
          />
          <Divider />
          <TextField
      ...
      <Paper zDepth={1} className={this.state.showResponse == true ? 
        'platforms-response col-md-8' : 
        'platforms-response-hidden col-md-8'} 
      >
      <p className="platforms-response-inside">{this.state.data}</p>
      </Paper>
      </div>
      );
  }
...
export default PlatformsSandbox;